 ...
 */
function formatDirections(object, color) {
...
function getDIRTooltip(self) {
  return [
    '<b>Sample: </b>' + self.point.sample,
    '<b>Declination: </b>' + self.x.toFixed(1),
...
var eqAreaDirections = function(siteName) {
  ...
  var chartOptions = {
    'chart': {
      'id': 'ChRM',
      'polar': true,
        'renderTo': 'magneticDirectionGraph'
      },
    'tooltip': {
      'formatter': function() {
        return getDIRTooltip(this);  
      }
    },
    'exporting': {
      'sourceWidth': 600,
      'sourceHeight': 600,
      'filename': siteName + '_DIR',
      'buttons': {
        'contextButton': {
          'symbolStroke': '#7798BF',
          'align': 'right'
 ...
 * Description: returns the tooltip for the VGP polar plot
 */
function getVGPTooltip(self) {
  return [
    '<b>Sample: </b>' + self.point.sample,
    '<b>Longitude: </b>' + self.x.toFixed(1),
    '<b>Latitude: </b>' + self.point.inc.toFixed(1),
    '<p><i> VGP positions are relative to the mean VGP </i>'
  ].join("<br>");
}
...
var eqAreaVGPs = function(name) {
  ...
  if(cutoff === 'none' ) {
    ...
    throw("Unknown cutoff for site");
  }
  
  // Construct the ellipse parameters using A95 (Fisher cone on poles)
  // around the North pole for VGP plot
  ...
  var chartOptions = {
    ...
    'series': [{
      ...
      'name': 'Mean', 
      'data': [{'sample': 'Mean', 'x': 0, 'y': eqArea(90), 'inc': 90}]
    }, {
      'type': 'line',
      'data': ellipse.neg,
      'enableMouseTracking': false,
...
/*
 * FUNCTION getMeanTooltip
 * Description: generates tooltip for mean direction polar plots
 */
function getMeanTooltip(self) {

  /* 
   * PRIVATE FUNCTION _createColorString
   * Description: creates a link to change marker color in Highcharts
   */
  function _createColorString(series, color) {
    return '<a onClick="module.map.changeColorFromName(\'' + series.name + '\', \'' + series.index + '\', \'' + color + '\')"><u>' + color +'</u></a>&nbsp';
  }

  var contentString = '';
  ['red', 'orange', 'blue', 'green', 'purple', 'pink'].forEach(function(x) {
    contentString += _createColorString(self.series, x);
  });

  return [
    '<b> Mean for Site: </b> ' + self.series.name,
    '<b> Mean Declination: </b>' + self.x.toFixed(1),
    '<b> Mean Inclination: </b>' + self.point.inc.toFixed(1),
    contentString
  ].join("<br>");
 ...
 * Output: VOID (calls Highcharts constructor)
 */
function plotMeans(siteData, container, title) {
        ...
        'point': {
          'events': {
            'click': function () {
              ...
              // Plot the expected declination on the map
...
function getReferenceFrameTooltip(self, title) {
  ...
  return [
    ...
    '<b>Plate: </b>' + self.series.name,
    '<b>Age: </b>' + self.x + ' Ma',
    '<b>Expected ' + title + ': </b>' + self.y.toFixed(2) + '°'
  ].join("<br>");
...
function EICDF (data, orig, inc, incAv, lower, upper) {
  ...
  // Only show the unflattened inclination if it has an intersection
  var text ='Original inclination [blue] of <b>' + orig.toFixed(2) + '°</b> has no intersection with the TK03.GAD polynomial';
  if(inc !== null) {
    mySeries.push({
...
/*
 * FUNCTION getEICDFTooltip
 * Description: formats tooltip for EI module CDF plot
 */
