  ...
  reader.onload = function() {
  	...
  	(addSitesTimed = function () {
  	  if(i < importData.data.length) {
  	  	...
  	  	notify('success', 'Importing was succesful; ' + j + ' new site(s) have been added.');
  	  	$("#loading").hide();
  	  	finishedLoading();
  	  }
  	})();
...
module.IO.table = function(siteNames) {
  ...
  if(siteNames.length == 0) {
  	notify('Failure', 'No data selected for exporting');
  	return;
  }
  	...
  	for(var i = 0; i < siteNames.length; i++) {
  	  ...
  	  if(sites[siteNames[i]].userInput.metaData.name != "TEMP") {
		  ...
		  if(parameter === "meanPole") {
  	  	...
  	  	var beddings = sites[key].userInput.data.map(function(entry) {
...
module.IO.exporting = function () {
  ...
  if(Object.keys(sites).length === 0 && Object.keys(APWPs).length === 0) {
    notify('note', 'There are no sites for exporting.')
  	return;
  }
  ...
  var exportData = {
  	'version': version,
	'dateExported': dateExported.toISOString(),
  	'apwp': APWPs,
  	'data': new Array()
  };
