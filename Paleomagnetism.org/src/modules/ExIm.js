...
module.IO.getSVG = function(id) {
	...
	for(var i = 0; i < charts.length; i++) {
		...
		svg = svg.replace('<svg', '<g transform="translate('+800*i+',0)" ');
		
		svg = svg.replace('</svg>', '</g>');
		svgArr.push(svg);
	}

    var svg = '<svg height="800" width="' + 800 * charts.length + '" version="1.1" xmlns="http://www.w3.org/2000/svg">' + svgArr.join('') + '</svg>';
	var form
