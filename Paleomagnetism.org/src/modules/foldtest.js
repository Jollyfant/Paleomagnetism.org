...
module.foldtest.extremes = function (siteNames) {
	...
	for(var j = 0; j < geographicData.length; j++) {
		...
		tectonicData.push(rotat(bedding+90, dip+90, geographicData[j]));
	}
 ...
 var unfoldData = function ( pseudoDirections, unfoldingMin, unfoldingMax) {
	...
	for(var j = unfoldingMin; j <= unfoldingMax; j += 10) {
		...
		for(var k = 0; k < pseudoDirections.length; k++) {
			tilts.push( new rotat( 90 + pseudoDirections[k][2], 90 + pseudoDirections[k][3]*0.01*j, pseudoDirections[k]));
		}
	...
	for(var j = -9; j <= 9; j++) {
		...
		if(index + j >= unfoldingMin && index + j <= unfoldingMax) {
			...
			tilts = new Array();
			for(var k = 0; k < pseudoDirections.length; k++) {
				tilts.push( new rotat( 90 + pseudoDirections[k][2], 90 + pseudoDirections[k][3]*0.01*(oldIndex+j), pseudoDirections[k]));
			}
