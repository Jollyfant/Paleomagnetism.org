...
$(function() {
  ...
  $("#initializeNTR, #iterationButton").button().click( function () {
    ...
    if(isRunning) {
      notify('failure', 'NTR Analysis is already running. Please wait'); return;
    }
    ...
    if(referencePole.dec === 0 && referencePole.inc === 0 && magnetizationVector.dec === 0 && magnetizationVector.inc === 0 && dykePole.dec === 0 && dykePole.inc === 0) {
      notify('failure', 'Please fill in the required input.'); return;
    }
...
function getIntersection( pole1, pole2 ) {
  ...
  return dir(cross.x, cross.y, cross.z);
...
function NTRAnalysis (referencePole, magnetizationVector, dykePole, iterate) {
  ...
  if(iterate) {
    ...
    magnetizationVectorEndPoints.push({
      ...
      'y': eqArea(magnetizationVector.inc),
  ...
  (function timed() {
    ...
    var type1 = cart(cross.dec, cross.inc).z < 0 ? 'CW' : 'CCW';
    ...
    var cross = getIntersection(m1, m2);
    var type2 = cart(cross.dec, cross.inc).z < 0 ? 'CW' : 'CCW';
    ...
    if(++i < iterationPermutations.length) {
      ...
      if(!iterate) {
        ...
        var kVector = cart(intS2.dec, intS2.inc);
        var crossProd = getCross(kVector, vVector);
        var dot = kVector.x * vVector.x + kVector.y * vVector.y + kVector.z * vVector.z;
      ...
      if(!iterate) {
        plotSeries.push({
          ...
          'data': [{
            'x': intercept.one.dec,
            'y': eqArea(intercept.one.inc),
            'inc': intercept.one.inc
          }, {
            'x': intercept.two.dec,
            'y': eqArea(intercept.two.inc),
