...
module.CTMD.initialize = function () {
  ...
  if(module.CTMD.running) {
    notify('failure', 'CTMD Module is already running.'); return;
  }
	
  // Require a minimu of two sites
  var siteName = $("#ctmdSel").val();
  if(siteName === null || siteName.length === 1) {
    notify('failure', 'Please select at least two sites.'); $("#CTMDHide").hide(); return;
  }
  ...
  var VGPFlag = $("#CTMDVGPRadio input[type='radio']:checked").val() === "VGP";
  ...
  (module.CTMD.timed = function() {
    ...
    if(j < (siteName.length) && i < (siteName.length)) {
      ...
      $("#CTMDHide").show();
      $("#CTMDPerc").html("Done!");
      notify('success', 'CTMD test has been completed!');
...
module.CTMD.sampleWatson = function (NI, KI) {
  ...
  return {
    ...
    'v99': WatsonParams[conf99],
    'v95': WatsonParams[conf95]
  }
...
module.CTMD.mean = function (N, coords) {
  ...
  var R = vectorLength(xSum, ySum, zSum);
