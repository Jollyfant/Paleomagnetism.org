...
function getPlate (plate, type) {
	...
	plate.africanPolePath = getAPWP(type);
...
function getExpectedLocation (skip) {
	...
	if(plates) {
		...
		for(var numPlates = 0; numPlates < plates.length; numPlates++) {
			...
			for( var i = 0; i < APWP.lon.length; i++) {
				if(APWP.age[i] >= ageMin && APWP.age[i] <= ageMax) {
					decErrors.push([APWP.age[i], (paleoDecs[i-skip].y - paleoDecs[i-skip].error), (paleoDecs[i-skip].y + paleoDecs[i-skip].error)]);	
					incErrors.push([APWP.age[i], (paleoIncs[i-skip].y - paleoIncs[i-skip].error), (paleoIncs[i-skip].y + paleoIncs[i-skip].error)]);
					palatErrors.push([APWP.age[i], (paleoLats[i-skip].y - paleoLats[i-skip].error[0]), (paleoLats[i-skip].y + paleoLats[i-skip].error[1])]);		
				} else {
					skip++;
				}
			}
